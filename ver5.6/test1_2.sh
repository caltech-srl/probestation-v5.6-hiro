...
if [ $RUN_LVL == "4" ]; then
   ...
   dialog --title "Probestation ASIC test" --menu "Select $ASIC scope test type: " 15 40 5 \
	1 "preamp" 2 "shaper" 3 "preamp (CP)" 2> _1.txt
   [ $? != 0 ] && exit
   RUN_OSC=$(cat _1.txt)
...
run_scope_sha() {
   ...
   if [ "$SSCAN" = "y" ]; then
      sleep 5
      ./xysel.sh "2 TRY5 " 700 >> $OUTFL$TYP_SH$SUF_OSC
...
run_scope_cpm() {
   ...
   if [ "$SSCAN" = "y" ]; then
      ...
      sleep 5
      ./single_xysel.sh 1 TRY5 16 16 >> $OUTFL$TYP_CP$SUF_OSC
   fi
}
